...
if (isset($_GET['action']) && $_GET['action'] == 'get_shift_details' && isset($_GET['id'])) {
    ...
    try {
        ...
        echo json_encode(['success' => true, 'movimientos' => $movimientos, 'turno' => $turno]);
    } catch (Exception $e) {
...
if ($isAdmin) {
    ...
    // MODIFICACION: Permitir que Cajero/Kiosquero vea historial global también
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-xl font-semibold">
                                ...
                                <?php echo ($isAdmin || !empty($usuarios)) ? 'Historial de Todos los Turnos' : 'Mis Turnos Anteriores'; ?>
                            </h3>
                            <?php if (($isAdmin || !empty($usuarios)) && !empty($usuarios)): ?>
                                <form method="GET" class="flex gap-2">
                                    <select name="user_filter" onchange="this.form.submit()"

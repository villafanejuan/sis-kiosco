            ...
            if (!empty($carrito)) {
                ...
                if ($stock_insufficient) {
                    ...
                    if ($amount_paid < $total) {
                        ...
                        // Insertar venta
                        $stmt = $pdo->prepare("INSERT INTO ventas (usuario_id, total, monto_pagado, cambio) VALUES (?, ?, ?, ?)");
                        $stmt->execute([$_SESSION['user_id'], $total, $amount_paid, $change]);
                        ...
                        unset($_SESSION['carrito']);
                        $carrito = [];
                        $message = '<div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">Venta completada exitosamente. Total: $' . number_format($total, 2) . ', Cambio: $' . number_format($change, 2) . '</div>';
                    }
                }
            } else {
                    ...
                    <div class="bg-white p-6 rounded-lg shadow-md">
                        ...
                        <div class="flex justify-between items-center mb-6 gap-4">
                            ...
                            <div class="flex bg-gray-100 p-1 rounded-lg border border-gray-200">
                                <button onclick="toggleView('grid')" id="btn_grid" class="p-2 rounded-md bg-white text-blue-600 shadow-sm transition">
                                    <i class="fas fa-th-large text-xl"></i>
                                </button>
                                <button onclick="toggleView('list')" id="btn_list" class="p-2 rounded-md text-gray-400 hover:text-gray-600 transition">
                        ...
                        <div id="products_list" class="hidden bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                                            ...
                                            <tr class="hover:bg-gray-50 transition product-row">
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="flex items-center">
                                                        <div class="flex-shrink-0 h-10 w-10 flex items-center justify-center bg-blue-100 text-blue-600 rounded-lg mr-4">
                                                            <i class="fas fa-box"></i>
                                                        </div>
                                                        <div class="text-sm font-medium text-gray-900 product-name"><?php echo htmlspecialchars($prod['nombre']); ?></div>
                                                    </div>
                                                </td>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $prod['stock'] > 10 ? 'bg-green-100 text-green-800' : ($prod['stock'] > 0 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'); ?>">
                                                        <span class="stock-display"><?php echo htmlspecialchars($prod['stock']); ?></span>
                                                    </span>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <div class="flex items-center justify-end space-x-2" data-product-id="<?php echo $prod['id']; ?>"> <!-- data-product-id for stock update scope -->
                                                        <input type="number" value="1" min="1" max="<?php echo $prod['stock']; ?>"
